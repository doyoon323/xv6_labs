...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->cwd = idup(p->cwd); //부모디렉토리, 프로세스 이름 복사 
 ...
 /*
  fork시 
  -> vruntime, nice, weight? 그대로 상속
  -> runtime =0, timeslice =5 로 초기화
  -> vdeadline 재계산 
  */
...
//Round-robin -> fair하지만, latency에 민감하지x
// -> deadline이 가장 빠른 process를 실행하는 EEVDF로 바꾸자

/*
원리:  
eligible한 프로세스 중에, vdeadline이 가장 이른 애부터 실행

eligible -> lag값으로 판단. lag >0 -> eligible
vruntime = actual_runtime × (1024 / weight) (vruntime이 낮을수록
 우선순위 높다)

vdeadline = vruntime + (time_slice × 1024 / weight)


EEVDF 작동흐름
1. vdeadline 설정(eligible하면, vdeadline설정)
2. vdeadline이 가장 작은 eligible process 설정(이건 스케쥴러?)
3. 실행 중 runtime,vruntime 없데이트(tick마다 runtime ++, vruntime +=(1tick * 1024/weight)
   time slice 끝 -> yield
4. 할당된 실행시간 끝 -> vdeadline, eligibility 게산 ->2로 돌아가

TODO
EEVDF 구현 -> ps() 출력

fork/wakeup -> vruntime/nice 유지 + timeslice 초기화+ vdeadline 재계산
*/
...
wakeup(void *chan)
{
  ...
  timeslice =5 초기화
  vdeadline, eligible 재계산 
  ****** sched() 절대 호출하지 말것, process timeslice끝날때까지 양보금지  *******
  */
  for(p = proc; p < &proc[NPROC]; p++) {
    if(p != myproc()){
